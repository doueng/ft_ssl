...
void	update_hash(t_hash *hash, uint32_t *w)
{
	uint32_t	h[8];
	uint32_t	tmp[8];
	size_t		i;
	...
	while (i < 64)
	{
		tmp[0] = rightrotate32(h[4], 6) ^ rightrotate32(h[4], 11) ^ rightrotate32(h[4], 25);
		tmp[1] = (h[4] & h[5]) ^ ((~h[4]) & h[6]);
		tmp[2] = h[7] + tmp[0] + tmp[1] + k[i] + w[i];
		tmp[3] = rightrotate32(h[0], 2) ^ rightrotate32(h[0], 13) ^ rightrotate32(h[0], 22);
...
void	prepare_words(uint32_t *w, uint8_t *msg, size_t offset)
{
	size_t i;
